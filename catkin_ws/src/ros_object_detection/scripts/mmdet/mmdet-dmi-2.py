...
class mmdet_node:
    ...
    def callback(self, frame):
        if frame.header.seq % 3 == 2:
            ...
            # test a single image
            # if rospy.get_param("mmdet_cuda_device") == 0:
            #     # img.to(torch.device("cuda:0"))
            #     result = inference_detector(self.model0, img)
            #     self.count += 1
            #     del img
            # elif rospy.get_param("mmdet_cuda_device") == 1:
            #     img.to(torch.device("cuda:1"))
            #     result = inference_detector(self.model1, img)
            #     self.count += 1
            #     del img
            # elif rospy.get_param("mmdet_cuda_device") == 2:
            #     img.to(torch.device("cuda:2"))
            #     result = inference_detector(self.model2, img)
            #     self.count += 1
            #     del img
            # elif rospy.get_param("mmdet_cuda_device") == 3:
            #     img.to(torch.device("cuda:3"))
            #     result = inference_detector(self.model3, img)
            #     self.count += 1
            #     del img
            # elif rospy.get_param("mmdet_cuda_device") == 4:
            #     # a = int(random.randint(0,3))
            #     # a = self.count*3 % 4
            #     model = choice([self.model0, self.model1, self.model2, self.model3])
            #     # models = [self.model0, self.model1, self.model2, self.model3]
            #     # print(a)
            #     # model = models[a]
            #     result = inference_detector(model, img)
