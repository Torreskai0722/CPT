...
class ProcManager:
	...
	def _check_priority(self, policy, priority):
		# Available priorities on Jetson AGX Xavier:
		...
		if policy in ["SCHED_OTHER", "SCHED_BATCH", "SCHED_IDLE", "SCHED_DEADLINE"]:
			if priority != 0:
				return("error priority for {}, should be 0 rather than {}".format(policy, priority))
		elif policy in ["SCHED_FIFO", "SCHED_RR"]:
			if priority < 1 or priority > 99:
				return("error priority for {}, should between 1 and 99, rather than {}".format(policy, priority))

	def set_scheduling_policy(self, pid, policy, priority):
		err0 = self._check_priority(self._policy_to_string(policy), priority)
		if err0: 
			return err0
	...
	def set_scheduling_policy_deadline(self, pid, runtime, deadline, period):
		cmd = "chrt -d -a -T {} -D {} -P {} -p 0 {}".format(runtime, deadline, period, pid)
		print(cmd)
		err = os.system(cmd)
		return err
		...
		while True:
			...
			if order['name'] == 'nice':
				...
				ret = self.set_scheduling_policy(order['pid'],
								 order['policy'],
								 order['priority'])
			elif order['name'] == 'scheduling_policy_deadline':
				ret = self.set_scheduling_policy_deadline(order['pid'],
								order['runtime'],
								order['deadline'],
        						order['period'])
				print(ret)
